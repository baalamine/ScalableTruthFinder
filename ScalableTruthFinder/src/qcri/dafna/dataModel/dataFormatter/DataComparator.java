...
public class DataComparator {
	...
	public static double computeImplication(ValueBucket bucket1, ValueBucket bucket2, ValueType valueType) {
		...
		if (DataTypeMatcher.savedAsString(valueType)) {
			...
			return similarityMetric.compare(bucket1.getCleanedString(), bucket2.getCleanedString())/(double)bucket2.getCleanedString().length();
		} 
...
//		}
	...
	public static double computeSimilarity(String s1, String s2) {
		JaroWinkler similarityMetric = new JaroWinkler();
		return similarityMetric.compare(s1, s2) ;
